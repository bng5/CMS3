...
if(isset($_GET['id']) && $item = @include(RUTA_CARPETA.'bng5/datos/item/'.$_GET['id'].'.'.$idioma->cod.'.php')) {
	$estilo = '';
			...
			<div id="seccion" class="proyectos">
				<div id="wrapper" class="wrapper">
					...
					<div class="ampliacion">
						...
						<span class="estrellas e'.$item["valores"][30].'">'.$estrellas_txt[$item["valores"][30]].'</span>
...
'.$item["valores"][9].'</p>
	...
	foreach($item["valores"][10]["imagenes"] AS $img) {
		echo '<td><img src="/img/0/10/'.$img['archivo'].'" width="'.$img['ancho'].'" height="'.$img['alto'].'" alt="" /></td>';
		$ancho += ($img['ancho'] + 11);
	}
	echo '
                      </tr>
...
?>
	...
	foreach($items AS $item) {
		...
		$items_txt .= '<td class="item">
				  <div class="itemInt">
					<div class="ol"><span class="d'.$num[0].'"></span><span class="u'.$num[1].'"></span><span class="simb s'.$k.'"></span></div>
					<h3><a name="'.$num.'" href="/'.$seccion->urls[$idioma->cod].'?id='.$item['id'].'&amp;ref='.$i.'">'.$item["string__titulo"].'</a></h3>
					<span class="estrellas e'.$item['int__estrellas'].'">'.$estrellas_txt[$item['int__estrellas']].'</span>
...
$(window).load(function() {
    if($(this).width() > 1086) {
        var padd = ($(this).width() - 1086) / 2;
        $(\'td:last\').css(\'paddingRight\', padd+\'px\');
    }
});
